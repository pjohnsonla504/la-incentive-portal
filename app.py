...
if check_password():
    ...
    def get_zoom_center(geoids):
        ...
        # Determine dynamic zoom based on bounding box spread
        lat_diff = max_lat - min_lat
        lon_diff = max_lon - min_lon
        max_diff = max(lat_diff, lon_diff)
        
        if max_diff == 0: zoom = 12.5 # Single tract close-up
    ...
    def render_map_go(df):
        ...
        selected_geoids = [rec['Census Tract Number'] for rec in (st.session_state["session_recs"] if isinstance(st.session_state["session_recs"], list) else [])]
        ...
        # Zoom logic: if a single tract is active, center on it. Otherwise center on the filtered set.
