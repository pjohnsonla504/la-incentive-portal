...
# --- 1. PERSISTENCE ENGINE (STABLE SYNC) ---
...
def save_rec_to_cloud(rec_entry):
    ...
    try:
        ...
        # Read latest cloud data (ttl=0 critical to prevent overwriting)
...
if check_password():
    ...
    def render_map_go(df):
        ...
        focus_geoids = {st.session_state["active_tract"]} if st.session_state.get("active_tract") and st.session_state["active_tract"] in map_df['geoid_str'].values else set(map_df['geoid_str'].tolist())
        ...
        fig.update_layout(
            ...
            legend=dict(title=dict(text="<b>Toggle Anchor Assets</b>"), yanchor="top", y=0.98, xanchor="left", x=0.02, bgcolor="rgba(255, 255, 255, 0.9)", font=dict(size=11, color="#1e293b"), bordercolor="#cbd5e1", borderwidth=1)
        )
        return fig
    ...
    if selected_parish != "All in Region":
        ...
        total_parish_tracts, eligible_oz_tracts = len(parish_data), len(parish_data[parish_data['Eligibility_Status'] == 'Eligible'])
        ...
        q_col2.markdown(f"<div class='metric-card'><div class='metric-value'>{eligible_oz_tracts}</div><div class='metric-label'>Eligible OZ Tracts</div></div>", unsafe_allow_html=True)
        q_col3.markdown(f"<div class='metric-card' style='border-color: #f97316;'><div class='metric-value' style='color:#f97316;'>{allowed_selections}</div><div class='metric-label'>Max (25% Limit)</div></div>", unsafe_allow_html=True)
    ...
    if st.session_state["active_tract"]:
        ...
        with d_col2:
            ...
            if curr in tract_centers:
                ...
                for _, a in working.sort_values('dist').head(5).iterrows():
                    st.markdown(f"<div class='anchor-card'><div style='display:flex; justify-content:space-between;'><span style='color:#f8fafc; font-weight:800; font-size:0.85rem;'>{a['Name']}</span><span style='color:#4ade80; font-size:0.7rem;'>{a['dist']:.1f} mi</span></div><div style='color:#94a3b8; font-size:0.7rem; margin-top:4px;'>{a['Type']}</div></div>", unsafe_allow_html=True)
    ...
    if st.session_state["session_recs"]:
        ...
        st.download_button("Download Report (CSV)", data=csv, file_name=f"OZ_2.0_Recommendations_{st.session_state['username']}.csv", mime='text/csv')
    else:
