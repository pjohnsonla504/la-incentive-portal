...
if check_password():
    ...
    st.markdown("""
        ...
        .section-title { font-size: 2.5rem; font-weight: 900; margin-bottom: 25px; line-height: 1.1; }
        .hero-title { font-size: 3.5rem; font-weight: 900; color: #f8fafc; margin-bottom: 20px; line-height: 1.1; }
        .hero-subtitle { color: #4ade80; font-size: 1.1rem; font-weight: 700; letter-spacing: 0.15em; text-transform: uppercase; margin-bottom: 10px;}
        .narrative-text { font-size: 1.1rem; color: #94a3b8; line-height: 1.6; max-width: 900px; margin-bottom: 30px; }
        .benefit-card { background-color: #111827 !important; padding: 30px; border: 1px solid #1e293b; border-radius: 12px; height: 100%; transition: all 0.3s ease; }
        .benefit-card:hover { border-color: #4ade80 !important; transform: translateY(-5px); background-color: #161b28 !important; }
        .benefit-card h3 { color: #f8fafc; font-size: 1.25rem; font-weight: 700; margin-bottom: 12px; }
    ...
    def render_map(df, isolated_id=None, is_filtered=False, height=600):
        ...
        # Highlight Logic
        if isolated_id:
            map_df['dynamic_opacity'] = map_df['geoid_str'].apply(lambda x: 1.0 if x == isolated_id else 0.15)
        else:
            map_df['dynamic_opacity'] = 0.6
        ...
        fig.update_traces(marker_opacity=map_df['dynamic_opacity'], marker_line_width=0.5)
        fig.update_layout(
    ...
    st.markdown("""
        <div class='content-section'>
            ...
            <div class='narrative-text'>
                Opportunity Zones 2.0 is Louisiana’s chance to turn bold ideas into real investment—unlocking long-term private capital to fuel jobs, small businesses, and innovation in the communities that need it most.
            </div>
        </div>
    """, unsafe_allow_html=True)

    # --- SECTION 2: BENEFIT FRAMEWORK ---
    st.markdown("<div class='content-section'><div class='section-num'>02 / FRAMEWORK</div><div class='section-title'>The OZ 2.0 Benefit Framework</div>", unsafe_allow_html=True)
    ...
    for i, (ct, ctx) in enumerate(cards2):
        c2[i].markdown(f"<div class='benefit-card'><h3>{ct}</h3><p>{ctx}</p></div>", unsafe_allow_html=True)

    # --- SECTION 3: ADVOCACY ---
    st.markdown("<div class='content-section'><div class='section-num'>03 / ADVOCACY</div><div class='section-title'>Census Tract Advocacy</div>", unsafe_allow_html=True)
    ...
    for i, (ct, ctx) in enumerate(cards3):
        c3[i].markdown(f"<div class='benefit-card'><h3>{ct}</h3><p>{ctx}</p></div>", unsafe_allow_html=True)

    # --- SECTION 4: BEST PRACTICES ---
    st.markdown("<div class='content-section'><div class='section-num'>04 / ELIGIBILITY</div><div class='section-title'>Statewide Eligibility Map</div>", unsafe_allow_html=True)
    st.markdown("<div class='narrative-text'>Explore the 1,148 census tracts across Louisiana. Green tracts indicate eligibility for Opportunity Zone 2.0 designation based on the Opportunity Zones Master File metrics.</div>", unsafe_allow_html=True)
    
    f4 = render_map(master_df, isolated_id=st.session_state["active_tract"])
    s4 = st.plotly_chart(f4, use_container_width=True, on_select="rerun", key="map4")
    if s4 and "selection" in s4 and s4["selection"]["points"]:
        new_id = str(s4["selection"]["points"][0]["location"])
    ...
    st.markdown("<div class='content-section'><div class='section-num'>05 / STRATEGIC MAPPING</div><div class='section-title'>Strategic Asset Mapping</div>", unsafe_allow_html=True)
    ...
    with c5b:
        ...
        if curr in tract_centers:
            ...
            for _, a in working_anchors.sort_values('dist').head(12).iterrows():
                link_btn = f"<div style='margin-top:10px;'><a href='{a['Link']}' target='_blank' style='display:inline-block; background:#4ade80; color:#0b0f19; padding:5px 12px; border-radius:4px; font-size:0.7rem; font-weight:900; text-decoration:none;'>VIEW SITE 🔗</a></div>" if a.get('Type') in ['Land', 'Buildings'] and a['Link'] != "" else ""
        ...
        components.html(f"<div style='height: 530px; overflow-y: auto; font-family: sans-serif;'>{list_html if list_html else 'No assets found.'}</div>", height=550)

    # --- SECTION 6: TRACT PROFILING ---
    st.markdown("<div class='content-section'><div class='section-num'>06 / ANALYSIS</div><div class='section-title'>Tract Profiling & Recommendations</div>", unsafe_allow_html=True)
    ...
    # --- SECTION 7: SUMMARY ---
    st.markdown("<div class='content-section'><div class='section-num'>07 / OUTPUT</div><div class='section-title'>My Recommended Tracts</div>", unsafe_allow_html=True)
