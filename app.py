...
if check_password():
    ...
    # --- 3. DATA ENGINE: FOCUS ON GEOID MATCHING ---
    ...
    def load_assets():
        ...
        # KEY FIX: Strictly clean the FIPS to match the JSON's GEOID strings
        master['geoid_str'] = master['11-digit FIP'].astype(str).str.split('.').str[0].str.zfill(11)
        
        # ELIGIBILITY: Highlight only OZ 2.0 Eligible tracts
        ...
        if geojson:
            for feature in geojson['features']:
                # The shapefile key is usually in properties.GEOID
                geoid = str(feature['properties'].get('GEOID', feature.get('id')))
                geom = feature['geometry']
                # Determine center for anchor distance logic
    ...
    st.markdown("<div class='content-section'><div class='section-num'>SECTION 1</div><div class='hero-subtitle'>Strategic Investment</div><div class='hero-title'>Louisiana Opportunity Zone 2.0</div><div class='narrative-text'>Connecting private capital to public purpose through data-driven insight.</div></div>", unsafe_allow_html=True)
    ...
    with c1: st.markdown("<div class='benefit-card'><h3>30% Rural Step-Up</h3><p>Enhanced basis for projects in non-metro census tracts.</p></div>", unsafe_allow_html=True)
    with c2: st.markdown("<div class='benefit-card'><h3>Capital Deferral</h3><p>Maximize liquidity with tax deferral through 2031.</p></div>", unsafe_allow_html=True)
    with c3: st.markdown("<div class='benefit-card'><h3>Permanent Exclusion</h3><p>Zero tax on the appreciation of your OZ 2.0 assets.</p></div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)

    st.markdown("<div class='content-section'><div class='section-num'>SECTION 3</div><div class='section-title'>Justification</div><div class='benefit-card'><h4>Anchor-Led Growth</h4><p>Targeting tracts within proximity to major healthcare and educational institutions.</p></div></div>", unsafe_allow_html=True)

    # --- SECTION 4: THE MAP (BOUNDARY FIX) ---
    st.markdown("<div class='content-section' style='border-bottom:none;'><div class='section-num'>SECTION 4</div><div class='section-title'>Strategic Tract Tool</div>", unsafe_allow_html=True)
    
    if gj:
        m_col, p_col = st.columns([7, 3])
        with m_col:
            # We specifically target 'properties.GEOID' to match our 'geoid_str'
            fig = px.choropleth_mapbox(
                master_df, 
                geojson=gj, 
                locations="geoid_str", 
                featureidkey="properties.GEOID", # Precision Join Key
                color="map_color", 
                color_discrete_map={1: "#4ade80", 0: "rgba(30,41,59,0.3)"},
                mapbox_style="carto-darkmatter", 
                zoom=6.2, 
                center={"lat": 31.0, "lon": -91.8},
                opacity=0.7
            )
            # FORCE BOUNDARIES: Ensuring shapefiles are visible
            fig.update_traces(
                marker_line_width=1.0, 
                marker_line_color="#475569", # Dark blue-gray boundary
                hovertemplate="<b>Tract: %{location}</b><extra></extra>"
            )
        ...
        with p_col:
            ...
            if not row.empty:
                d = row.iloc[0]
                st.markdown(f"<h3>Tract {current_id}</h3><p style='color:#4ade80;'><b>{d.get('Parish', 'Louisiana').upper()} PARISH</b></p>", unsafe_allow_html=True)
                
                # Metrics from Opportunity Zones Master File
                st.markdown(f"<div class='metric-card'><div class='metric-value'>{d.get('Estimate!!Percent below poverty level!!Population for whom poverty status is determined', 'N/A')}%</div><div class='metric-label'>Poverty Rate</div></div>", unsafe_allow_html=True)
        ...
        st.error("GeoJSON not loading. Check your internet connection or the URL source.")
