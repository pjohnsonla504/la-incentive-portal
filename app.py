...
def load_data():
    ...
    # Standardize GEOID (Column B / Index 1)
    geoid_col = df.columns[1]
    df['GEOID_KEY'] = df[geoid_col].astype(str).str.replace(r'\.0$', '', regex=True).str.zfill(11)
    
    # Map the 11 Metrics to your specific headers
    m_map = {
    ...
    with col2:
        with st.form("login"):
            ...
            if st.form_submit_button("Login"):
                ...
                if not match.empty:
                    st.session_state.update({"authenticated": True, "username": u, "role": str(match.iloc[0]['Role']), 
