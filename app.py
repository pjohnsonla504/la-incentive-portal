...
if check_password():
    ...
    def load_assets():
        ...
        # Updated to V2 file per instructions
        ...
        # Highlighting logic: Green for OZ 2.0 Eligible (Column E)
        ...
        # OZ 1.0 logic for mapping (Column D)
        master['OZ_1_Status'] = master['Current OZ 1.0 tract (expires Dec. 2028)'].apply(
    ...
    def render_map_go(df):
        ...
        fig.add_trace(go.Choroplethmapbox(
            ...
            hoverinfo="location",
            name="OZ 2.0 Eligibility (Green)"
        ))

        # TOGGLE LAYER: OZ 1.0 (Blue Highlight)
        oz1_df = map_df[map_df['OZ_1_Status'] == 'Yes']
        fig.add_trace(go.Choroplethmapbox(
            geojson=gj,
            locations=oz1_df['geoid_str'],
            z=[1]*len(oz1_df),
            featureidkey="properties.GEOID" if "GEOID" in str(gj) else "properties.GEOID20",
            colorscale=[[0, '#3b82f6'], [1, '#3b82f6']],
            showscale=False,
            marker=dict(opacity=0.4, line=dict(width=1, color='blue')),
            name="Current OZ 1.0 (2028)",
            visible="legendonly"
        ))
        ...
        fig.update_layout(
            ...
            legend=dict(
                title=dict(text="<b>Map Layers & Anchors</b>", font=dict(size=12)),
    ...
    if st.session_state["active_tract"]:
        ...
        with d_col1:
            ...
            m1 = st.columns(3)
            # Pulling from Column G (Rural Eligibility) as Metro Status
            m1[0].markdown(f"<div class='metric-card'><div class='metric-value'>{row.get('Rural Eligibility for OZ 2.0', 'N/A')}</div><div class='metric-label'>Metro Status</div></div>", unsafe_allow_html=True)
            m1[1].markdown(f"<div class='metric-card'><div class='metric-value'>{row.get('NMTC Basic Eligibility', 'N/A')}</div><div class='metric-label'>NMTC Eligible</div></div>", unsafe_allow_html=True)
            m1[2].markdown(f"<div class='metric-card'><div class='metric-value'>{row.get('Eligibility for OZ 2.0 Designation', 'N/A')}</div><div class='metric-label'>OZ 2.0 Status</div></div>", unsafe_allow_html=True)
            ...
            m2[1].markdown(f"<div class='metric-card'><div class='metric-value'>${safe_float(row.get('Median Household Income', 0)):,.0f}</div><div class='metric-label'>Income</div></div>", unsafe_allow_html=True)
            ...
            if st.button("Add to Recommendation Report", use_container_width=True, type="primary"):
                st.session_state["session_recs"].append({
                    ...
                    "Income": f"${safe_float(row.get('Median Household Income', 0)):,.0f}",
        ...
        with d_col2:
            ...
            if curr in tract_centers:
                ...
                for _, a in working.sort_values('dist').head(15).iterrows():
                    type_color = "#4ade80"
                    list_html += f"""<div class='anchor-card'>
