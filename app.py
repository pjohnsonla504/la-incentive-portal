...
def check_password():
    def password_entered():
        try:
            ...
            u = st.session_state["user_input"].strip()
            p = str(st.session_state["pass_input"]).strip()
    ...
    if not st.session_state["password_correct"]:
        ...
        with col_mid:
            ...
            """, unsafe_allow_html=True)
            with st.container():
                st.text_input("Username", key="user_input", placeholder="Enter your username")
                st.text_input("Password", type="password", key="pass_input", placeholder="••••••••")
...
if check_password():
    ...
    # (Existing benefits/strategy/best practices sections omitted for brevity but remain in full version)
    ...
    if st.session_state["active_tract"]:
        ...
        with d_col1:
            ...
            rec_cat = st.selectbox("Recommendation Category", ["Housing Development", "Business Development", "Technology & Research", "Healthcare & Community Services"], key="rec_cat_input")
            justification = st.text_area("Strategic Justification", height=120, key="just_input")
    ...
    if st.session_state["session_recs"]:
        ...
        with c1:
            if st.button("Submit Report to Google Sheets", use_container_width=True, type="primary"):
                try:
                    conn = st.connection("gsheets", type=GSheetsConnection)
                    # Use the username as the worksheet name
                    sheet_name = st.session_state["username"]
                    
                    # Try to read existing data to check if sheet exists
                    try:
                    ...
                    st.success(f"Report successfully submitted to worksheet: {sheet_name}")
                    st.session_state["session_recs"] = []
                except Exception as e:
