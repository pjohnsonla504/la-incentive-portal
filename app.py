...
with col3:
    submissions = len(df[df['Justification'].notna()])
    st.metric("Recommendations Filed", submissions)

# 5. Map Isolation & Census Map
st.divider()
st.subheader("Census Map & OZ Eligibility")

# Filter for Map Isolation
filter_oz = st.checkbox("Isolate Opportunity Zone 2.0 Tracks")
...
fig = px.choropleth_mapbox(
    ...
    geojson=la_geojson,
    locations="Tract",
    featureidkey="properties.GEOID", # Adjust based on your JSON structure
    color="OZ_Status",
...
def highlight_oz(row):
    # Tracks highlighted green are only those eligible for OZ 2.0 [cite: 2026-01-22]
