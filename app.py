...
if not st.session_state["authenticated"]:
    ...
    with st.form("login"):
        ...
        if st.form_submit_button("Login"):
            ...
            match = db[(db['Username'].astype(str)==u.strip()) & (db['Password'].astype(str)==p.strip())]
            if not match.empty:
...
def load_all():
    ...
    try:
        ...
        a.columns = [c.strip().lower() for c in a.columns] # Handle Name, Lat, Lon, Type
...
# --- 4. UI ---
st.title(f"📍 OZ 2.0: {st.session_state['a_val']}")
...
with c1:
    ...
    only_e, show_a = f_e.toggle("Eligible Only"), f_a.toggle("Show Anchors", True)
    ...
    if show_a and not anchor_df.empty:
        fig.add_scattermapbox(lat=anchor_df['lat'], lon=anchor_df['lon'], mode='markers',
                              marker=dict(size=12, color='#1E90FF'), text=anchor_df['name'], below='')
...
with c2:
    ...
    m_c = st.columns(3)
    m_c[0].metric("Pop", f"{disp['pop_total']:,.0f}")
    m_c[1].metric("Inc", f"${disp['med_hh_income']:,.0f}")
    m_c[2].metric("Pov", f"{disp['poverty_rate']}%")
    st.write(f"Unemp: {disp['unemp_rate']}% | 18-24: {disp['age_18_24_pct']}% | HS: {disp['hs_plus_pct_25plus']}% | BA: {disp['ba_plus_pct_25plus']}%")
    
    def glow(l, a, c):
        bg = c if a else "#343a40"
        return f'<div style="background-color:{bg}; padding:5px; border-radius:5px; text-align:center; color:white; font-size:10px; font-weight:bold; margin:2px;">{l}</div>'
    
    b1, b2 = st.columns(2)
    b1.markdown(glow("URBAN", (disp['is_rural']==0 and gid), "#dc3545"), unsafe_allow_html=True)
    b1.markdown(glow("RURAL", (disp['is_rural']==1 and gid), "#28a745"), unsafe_allow_html=True)
    b2.markdown(glow("NMTC ELIGIBLE", (disp['nmtc_eligible']==1 and gid), "#28a745"), unsafe_allow_html=True)
    b2.markdown(glow("DEEP DISTRESS", (disp['deep_distress']==1 and gid), "#28a745"), unsafe_allow_html=True)

    with st.form("sub"):
        st.write(f"GEOID: {gid}")
        cat, txt = st.selectbox("Type", ["Housing","Health","Infra","Comm","Other"]), st.text_area("Justification")
