...
if check_password():
    ...
    def load_assets():
        ...
        # Required columns check (prevents silent 0s)
        required_cols = [
            "GEOID",
            "Parish",
            "Region",
        ...
        if missing:
            raise ValueError(f"Master V2 CSV missing required columns: {missing}")

        # GEOID -> geoid_str (robust to floats like 22033000100.0)
        master["geoid_str"] = (
            master["GEOID"]
            .astype(str)
        ...
        # Highlighting driven by "Eligibility for OZ 2.0 Designation"
        master["Eligibility_Status"] = master["Eligibility for OZ 2.0 Designation"].apply(
            lambda x: "Eligible"
            if str(x).strip().lower() in ["eligible", "yes", "1", "true"]
            else "Ineligible"
        )

        # NMTC status from flags
        def nmtc_status_from_flags(row):
            deep = str(row.get("NMTC Deep Distress Eligibility", "")).strip().lower() in [
                "yes",
                "eligible",
                "1",
                "true",
            ...
            basic = str(row.get("NMTC Basic Eligibility", "")).strip().lower() in [
                "yes",
                "eligible",
                "1",
                "true",
        ...
        # --- TRACT CENTERS (for distance calc) ---
        centers = {}
        if gj:
            for feature in gj["features"]:
                ...
                try:
                    ...
                    if geom["type"] == "Polygon":
                        ...
                        continue
                    centers[str(geoid).zfill(11)] = [np.mean(pts[:, 0]), np.mean(pts[:, 1])]
                except Exception:
                    continue
    ...
    def get_zoom_center(geoids):
        ...
        for feature in gj["features"]:
            ...
            gid = str(gid).replace(".0", "").strip().zfill(11)
            if gid in geoids:
                geom = feature["geometry"]
                if geom["type"] == "Polygon":
    ...
    with p_col2:
        st.markdown(
            "<div class='benefit-card'><h3>Frost Brown Todd</h3><p>Craft a strategy that supports diverse project types, including commercial, industrial, and mixed-use developments.</p><a href='https://fbtgibbons.com/strategic-selection_of_opportunity_zones_2_0_a_governors_guide_to_best_practices/' target='_blank'>Strategic Selection Guide ↗</a></div>",
            unsafe_allow_html=True,
        )
    with p_col3:
        st.markdown(
    ...
    if st.session_state["active_tract"]:
        ...
        with d_col1:
            ...
            m1 = st.columns(3)
            # Replaced Metro Status tile (not in V2) with Rural Eligibility flag
            m1[0].markdown(
        ...
        with d_col2:
            ...
            if curr in tract_centers:
                ...
                st.info("Tract center not available (GeoJSON missing or GEOID mismatch).")
