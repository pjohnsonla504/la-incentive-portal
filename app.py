...
def load_master_data():
    ...
    # Ensure GEOID is a clean 11-digit string
...
with col2:
    # Eligibility logic [cite: 2026-01-22]
    eligible_count = len(master_df[master_df['Is_Eligible'] == 1])
    st.metric("Eligible (OZ 2.0)", eligible_count)
with col3:
...
with st.expander("Filter and Isolate Data", expanded=True):
    iso_col1, iso_col2 = st.columns(2)
    with iso_col1:
        # REDO Region Isolation [cite: 2026-01-22]
...
with st.form("recommendation_form", clear_on_submit=True):
    ...
    if submit_btn:
        if not justification:
            ...
            try:
                # Find eligibility from Master CSV
                ...
                st.success(f"✅ Saved recommendation for {selected_geoid}!")
                st.balloons()
            except Exception as e:
