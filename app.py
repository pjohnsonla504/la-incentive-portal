...
if check_password():
    ...
    gj, master_df, anchors_df = load_portal_data()
    ...
    # --- SECTION 5: ASSET MAP (Static Visualization) ---
    ...
    if gj:
        fig_5 = px.choropleth_mapbox(
            ...
            mapbox_style="open-street-map", zoom=6, center={"lat": 31.0, "lon": -92.0}, opacity=0.5
        )
        ...
        st.plotly_chart(fig_5, use_container_width=True, key="static_map_section_5")

    # --- SECTION 6: RECOMMENDATION TOOL (Interactive) ---
    st.markdown("""
    ...
    if gj:
        ...
        with m_col:
            fig_6 = px.choropleth_mapbox(
                ...
                mapbox_style="open-street-map", zoom=6.5, center={"lat": 30.8, "lon": -91.8}, opacity=0.7
            )
            fig_6.update_layout(coloraxis_showscale=False, margin={"r":0,"t":0,"l":0,"b":0}, paper_bgcolor='rgba(0,0,0,0)', height=600)
            fig_6.update_traces(marker_line_width=0.7, marker_line_color="#475569", showlegend=False)
            selection = st.plotly_chart(fig_6, use_container_width=True, on_select="rerun", key="interactive_map_section_6")
        ...
        with p_col:
            ...
            if not row.empty:
                ...
                st.markdown(f"<p style='color:#4ade80; font-weight:800; font-size:1.2rem;'>{str(d.get('Parish', 'LOUISIANA')).upper()}</p>", unsafe_allow_html=True)
                ...
                st.write("---")
                justification = st.text_area("Narrative Input", label_visibility="collapsed", placeholder="Describe why this tract is a priority...", height=150, key="justification_area")
                if st.button("Log Recommendation", use_container_width=True, type="primary"):
                    if justification:
                        if current_id not in [x['Tract'] for x in st.session_state["recommendation_log"]]:
