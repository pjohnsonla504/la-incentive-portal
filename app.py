...
if check_password():
    ...
    def load_assets():
        ...
        if gj and gj.get("features"):
            # detect correct GeoJSON GEOID property key
            props0 = gj["features"][0].get("properties", {})
            if "GEOID" in props0:
                gj_id_prop = "GEOID"
            elif "GEOID20" in props0:
                gj_id_prop = "GEOID20"
            else:
                gj_id_prop = None
            ...
            if gj_id_prop:
                for feature in gj["features"]:
                    geoid = feature["properties"].get(gj_id_prop)
                    try:
                        ...
                        if geom["type"] == "Polygon":
                            ...
                            continue
                        centers[str(geoid).replace(".0", "").strip().zfill(11)] = [
                            np.mean(pts[:, 0]),
                            np.mean(pts[:, 1]),
                        ]
                    except Exception:
    ...
    # --- Determine which GEOID field the GeoJSON uses ---
    GJ_ID_PROP = None
    if gj and gj.get("features"):
        props = gj["features"][0].get("properties", {})
        if "GEOID" in props:
            GJ_ID_PROP = "GEOID"
        elif "GEOID20" in props:
            GJ_ID_PROP = "GEOID20"
    ...
    def render_map_go(df):
        ...
        fig.add_trace(
            go.Choroplethmapbox(
                ...
                featureidkey=f"properties.{GJ_ID_PROP or 'GEOID'}",
    ...
    if gj is None or GJ_ID_PROP is None:
        st.error("GeoJSON not loaded or GEOID field not found (expected properties.GEOID or properties.GEOID20).")
