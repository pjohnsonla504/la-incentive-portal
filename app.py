...
if check_password():
    ...
    # --- SECTIONS 1-4: FRAMEWORK ---
    st.markdown("<div class='content-section'><div class='section-num'>SECTION 1</div><div class='hero-subtitle'>Opportunity Zones 2.0</div><div class='hero-title'>Louisiana Opportunity Zone 2.0 Recommendation Portal</div></div>", unsafe_allow_html=True)
    ...
    # --- SECTION 6: THE PERFECT NINE METRICS ---
    ...
    with c6b:
        ...
        if not row.empty:
            d = row.iloc[0]
            st.markdown(f"<div class='tract-header-container'><div class='header-parish'>{str(d.get('Parish','')).upper()}</div><div style='color:#94a3b8; font-size:0.8rem; font-weight:700;'>TRACT: {st.session_state['active_tract']}</div></div>", unsafe_allow_html=True)
