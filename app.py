...
if check_password():
    ...
    # --- 2. GLOBAL STYLING (Matched Heights & Sidebar Layout) ---
    st.markdown("""
        <style>
        @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;700;900&family=Playfair+Display:ital,wght@0,900;1,900&display=swap');
        ...
        .anchor-pill { background: #1f2937; border: 1px solid #374151; padding: 15px; border-radius: 8px; margin-bottom: 12px; }
        .anchor-pill:hover { border-color: #4ade80; background: #263345; }
        .anchor-type { color: #4ade80; font-size: 0.7rem; font-weight: 900; text-transform: uppercase; letter-spacing: 0.1em; }
        .anchor-name { font-weight: 700; font-size: 1rem; color: #f8fafc; display: block; margin: 4px 0; }
        .anchor-dist { color: #94a3b8; font-size: 0.8rem; }
        </style>
        """, unsafe_allow_html=True)
    ...
    # --- SECTION 5: SIDE-BY-SIDE ASSET MAPPING (FIXED LAYOUT) ---
    ...
    col5_map, col5_list = st.columns([6.5, 3.5])
    ...
    with col5_map:
        ...
        sel5 = st.plotly_chart(fig5, use_container_width=True, on_select="rerun", key="map_v5_side")
        if sel5 and sel5.get("selection", {}).get("points"): st.session_state["active_tract"] = str(sel5["selection"]["points"][0]["location"])

    with col5_list:
        curr = st.session_state["active_tract"]
        st.markdown(f"<h4>Anchor Assets near {curr}</h4>", unsafe_allow_html=True)
        ...
        if curr in tract_centers:
            ...
            # Displaying Name and Type clearly
            for _, a in anchors_df.sort_values('dist').head(25).iterrows():
                a_type = str(a.get('Type', 'Infrastructure Asset'))
                st.markdown(f"""
                <div class='anchor-pill'>
                    <span class='anchor-type'>{a_type}</span>
                    <span class='anchor-name'>{a['Name']}</span>
                    <span class='anchor-dist'>📍 {a['dist']:.1f} miles from center</span>
                </div>
                """, unsafe_allow_html=True)
    ...
    with col6_map:
        ...
        sel6 = st.plotly_chart(fig6, use_container_width=True, on_select="rerun", key="map_v6_side")
