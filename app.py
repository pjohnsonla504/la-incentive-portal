...
if check_password():
    ...
    def load_assets():
        geojson = None
        # Primary local load, fallback to raw GitHub source
        ...
        # Green only for Opportunity Zone 2.0 Eligible
    ...
    st.markdown("<div class='narrative-text'>Regional driven advocacy to amplify local stakeholder needs.</div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='narrative-text'>Leverage OZ 2.0 capital to catalyze community and economic development. Stack incentives like Historic Tax Credits and LIHTC.</div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)

    # --- SECTION 5: ASSET MAP ---
    if gj is None:
        st.error("Map Data (GeoJSON) failed to load. Please check your internet connection or the 'tl_2025_22_tract.json' file.")
    else:
        ...
        with a_col_left:
            # Using 'carto-darkmatter' which is built-in and highly reliable
        ...
        with a_col_right:
            ...
            st.markdown(f"<div class='metric-card'><div class='metric-value'>{len(local_assets)}</div><div class='metric-label'>Nearby Assets</div></div>", unsafe_allow_html=True)
            
            for _, asset in local_assets.head(5).iterrows():
                st.markdown(f"<div class='asset-item'><b>{asset['Name']}</b><br><small>{asset.get('Type', 'Asset')}</small></div>", unsafe_allow_html=True)
    ...
    with m_col:
        ...
        fig_rec.update_layout(margin={"r":0,"t":0,"l":0,"b":0}, paper_bgcolor='rgba(0,0,0,0)', height=500, showlegend=False)
    ...
    with p_col:
        ...
        if rec_id:
            ...
            if not row.empty:
                d = row.iloc[0]
                st.markdown(f"### Priority Tract: {rec_id}")
                st.markdown(f"**Parish:** {d.get('Parish', 'N/A')}")
                justification = st.text_area("Justification", placeholder="Why this tract?")
                if st.button("Log Selection", use_container_width=True, type="primary"):
