...
with col_side:
    ...
    if not match.empty:
        ...
        if t_pos:
            ...
            table_html = "<table class='anchor-table'><tr><th>DISTANCE</th><th>NAME</th><th>TYPE</th></tr>"
            for _, a in top5.iterrows():
                table_html += f"<tr><td>{a['d']:.1f} mi</td><td>{a['name'].upper()}</td><td>{str(a.get('type','N/A')).upper()}</td></tr>"
            table_html += "</table>"
