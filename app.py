...
if check_password():
    ...
    st.markdown("""
        ...
        .tract-header-container { 
            ...
            border-top: 4px solid #4ade80; 
            margin-bottom: 15px;
            border-left: 1px solid #1e293b; border-right: 1px solid #1e293b; border-bottom: 1px solid #1e293b;
        }
    ...
    def load_assets():
        ...
        # Loading Master File for metrics and eligibility
        master = read_csv_safe("Opportunity Zones 2.0 - Master Data File.csv")
        master['geoid_str'] = master['11-digit FIP'].astype(str).str.split('.').str[0].str.zfill(11)
        
        # Rule: Only tracks highlighted green are those eligible for OZ 2.0
    ...
    def render_map(df, height=600):
        # Eligible = Green, Ineligible = Light Slate
    ...
    st.markdown("""
        <div class='content-section'>
            ...
            <div class='hero-title'>Louisiana OZ 2.0 Recommendation Portal</div>
            <div class='narrative-text'>Unlocking long-term private capital to fuel jobs and innovation.</div>
        </div>
    """, unsafe_allow_html=True)
    ...
    with c5b:
        ...
        list_html = "<style>.anchor-item { background:#111827; border:1px solid #1e293b; padding:12px; border-radius:8px; margin-bottom:10px; }</style>"
        if curr in tract_centers:
            ...
            for _, a in anchors_df.sort_values('dist').head(10).iterrows():
                list_html += f"<div class='anchor-item'><div style='color:#4ade80; font-size:0.65rem; font-weight:900;'>{str(a.get('Type','')).upper()}</div><div style='font-weight:700; color:#f8fafc;'>{a['Name']}</div><div style='color:#94a3b8; font-size:0.75rem;'>📍 {a['dist']:.1f} miles</div></div>"
    ...
    if not row.empty:
        d = row.iloc[0]
        st.markdown(f"<div class='tract-header-container'><div class='header-parish'>{str(d.get('Parish','')).upper()}</div></div>", unsafe_allow_html=True)
        r1 = st.columns(3)
        r1[0].markdown(f"<div class='metric-card'><div class='metric-value'>{d.get('Unemployment Rate (%)','0')}%</div><div class='metric-label'>Unemployment</div></div>", unsafe_allow_html=True)
        r1[1].markdown(f"<div class='metric-card'><div class='metric-value'>Eligible</div><div class='metric-label'>OZ 2.0 Status</div></div>" if d['Eligibility_Status'] == 'Eligible' else "<div class='metric-card'><div class='metric-value'>Ineligible</div><div class='metric-label'>OZ 2.0 Status</div></div>", unsafe_allow_html=True)
        r1[2].markdown(f"<div class='metric-card'><div class='metric-value'>{d.get('Broadband Internet (%)','0')}%</div><div class='metric-label'>Broadband</div></div>", unsafe_allow_html=True)
